...
public class OrderServlet  extends BaseServlet{
	 ...
	 * @throws IOException
	 */
	public String carorder(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		User user=(User) request.getSession().getAttribute("user");
		if(user == null){
			//未登录 提示
			request.setAttribute("msg", "请先登录!");
		...
		try {
			if(request.getParameter("ordertime")==null||request.getParameter("ordertime").equals("")){	
				Date day=new Date();    
				SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
				
				order.setOrdertime(df.format(day));
				order.setExplain("申请VIP,成为公司的VIP客户");
				order.setType("VIP注册");
			}else{
